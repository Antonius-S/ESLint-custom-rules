 ...
 * Mod by A_S: "ignoreConstDeclaration: boolean" property to allow numbers in declaration of consts
 * no-magic-numbers: ["error", { "ignoreConstDeclaration": true }]
 * const MSEC_PER_MIN = 60 * 1000 => OK
 * const DEF_OPTS = { timeout: 10 * MSEC_PER_MIN } => OK
 * `let` assignments are still disallowed
 */
...
module.exports = {
    ...
    create(context) {
        const config = context.options[0] || {},
            ...
            ignoreConstDeclaration = !!config.ignoreConstDeclaration;
        ...
        function isInConstDeclaration(fullNumberNode) {
            let node = fullNumberNode.parent;
            while (node) {
        ...
        return {
            Literal(node) {
				...
				// Allow in constant declaration
				if (ignoreConstDeclaration && isInConstDeclaration(fullNumberNode))
					return;
